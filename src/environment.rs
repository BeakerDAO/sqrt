...
use radix_engine::types::{
    count, manifest_decode, ComponentAddress, Decimal, Encoder, ManifestEncoder, ManifestValueKind,
    NonFungibleLocalId, MANIFEST_SBOR_V1_MAX_DEPTH, MANIFEST_SBOR_V1_PAYLOAD_PREFIX,
};
    ...
    fn encode(
        ...
        match self {
            ...
            Environment::FungibleBucket(resource_name, amount) => {
                ...
                let (_, bucket, _) =
                    manifest_builder.add_instruction(Instruction::TakeFromWorktopByAmount {
                        amount: amount.clone(),
                        resource_address,
                    });
            ...
            Environment::NonFungibleBucket(resource_name, ids) => {
                ...
                let (_, bucket, _) =
                    manifest_builder.add_instruction(Instruction::TakeFromWorktopByIds {
                        ids: set_ids,
                        resource_address,
                    });
                encoder.encode(&(bucket.unwrap())).unwrap();
            ...
            Environment::FungibleProof(resource_name, amount) => {
                ...
                let (_, _, proof) = manifest_builder.add_instruction(
            ...
            Environment::NonFungibleProof(resource_name, ids) => {
                ...
                let (_, _, proof) =
                    manifest_builder.add_instruction(Instruction::CreateProofFromAuthZoneByIds {
                        ids: set_ids,
                        resource_address,
                    });
                encoder.encode(&(proof.unwrap())).unwrap();
