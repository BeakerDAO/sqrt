...
pub fn get_outcome_and_fees(receipt: &TransactionReceipt) -> (Option<Decimal>, Outcome) {
    match receipt.result.clone() {
        TransactionResult::Commit(commit_result) => (
            Some(commit_result.fee_summary.total_execution_cost_xrd.clone()),
            Outcome::Success,
        ),
        TransactionResult::Reject(rejected) => {
            let outcome = match rejected.error {
                RejectionError::ErrorBeforeFeeLoanRepaid(error) => match error {
                    RuntimeError::KernelError(kerror) => match kerror {
                        KernelError::WasmRuntimeError(wasm_error) => match wasm_error {
